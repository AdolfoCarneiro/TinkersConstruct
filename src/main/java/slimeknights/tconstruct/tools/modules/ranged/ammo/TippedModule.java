...
public enum TippedModule implements ModifierModule, ProjectileLaunchModifierHook.NoShooter, ProjectileHitModifierHook, ModifierRemovalHook, DisplayNameModifierHook, TooltipModifierHook {
  ...
  public boolean onProjectileHitEntity(ModifierNBT modifiers, ModDataNBT persistentData, ModifierEntry modifier, Projectile projectile, EntityHitResult hit, @Nullable LivingEntity attacker, @Nullable LivingEntity target) {
    ...
    if (target != null && persistentData.contains(key, Tag.TAG_STRING)) {
      ...
      if (id != null) {
        ...
        Potion potion = BuiltInRegistries.POTION.getHolder(id).map(h -> h.value()).orElse(null);
        if (potion != null) for (MobEffectInstance instance : potion.getEffects()) {
      ...
      if (id != null) {
        BuiltInRegistries.POTION.getHolder(id).ifPresent(potionHolder -> {
          if (!potionHolder.value().getEffects().isEmpty()) {
            new PotionContents(potionHolder).addPotionTooltip(tooltip::add, 1f / getDivisor(modifier), 20f);
          }
        });
      }
    }
  ...
  public Component getDisplayName(IToolStackView tool, ModifierEntry entry, Component name, @Nullable RegistryAccess access) {
    ...
    if (toolData.contains(key, Tag.TAG_STRING)) {
      ResourceLocation id = ResourceLocation.tryParse(toolData.getString(key));
      if (id != null) {
        var potionOpt = BuiltInRegistries.POTION.getHolder(id);
