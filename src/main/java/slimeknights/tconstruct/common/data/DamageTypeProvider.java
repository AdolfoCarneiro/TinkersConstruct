...
public class DamageTypeProvider implements RegistrySetBuilder.RegistryBootstrap<DamageType> {
  ...
  public void run(BootstrapContext<DamageType> context) {
    context.register(SMELTERY_HEAT, new DamageType("smeltery_heat", DamageScaling.NEVER, 0.1f, DamageEffects.BURNING));
    context.register(SMELTERY_MAGIC, new DamageType("smeltery_magic", DamageScaling.NEVER, 0.1f, DamageEffects.BURNING));
    context.register(KNIGHTMETAL, new DamageType("knightmetal", DamageScaling.NEVER, 0.1f));
    context.register(UPDATE_HEALTH, new DamageType("update_health", DamageScaling.NEVER, 0f, DamageEffects.DROWNING));
    context.register(THROWN_TOOL, new DamageType("thrown_tool", 0.1f));
    context.register(MELEE_THROWN_TOOL, new DamageType("thrown_tool", 0.1f));
    context.register(FISHING_HOOK, new DamageType("fishing_hook", 0.1f));
    context.register(MELEE_FISHING_HOOK, new DamageType("fishing_hook", 0.1f));
    context.register(PIERCING, new DamageType("piercing", 0.1f));
    context.register(BLEEDING, new DamageType("bleed", DamageScaling.NEVER, 0.1f));
    context.register(ENTANGLED, new DamageType("entangled", DamageScaling.NEVER, 0.1f));
    context.register(SPINY, new DamageType("spiny", DamageScaling.NEVER, 0.1f));
    context.register(SHOCK, new DamageType("shock", 0.1f, DamageEffects.BURNING));
    context.register(SELF_DESTRUCT, new DamageType("self_destruct", DamageScaling.NEVER, 0.1f));
    ...
    // fluid effects
    register(context, FLUID_IMPACT, new DamageType("fluid.impact", 0.1f, DamageEffects.HURT));
    register(context, FLUID_FIRE, new DamageType("fluid.fire", 0.1f, DamageEffects.BURNING));
    register(context, FLUID_COLD, new DamageType("fluid.cold", 0.1f, DamageEffects.FREEZING));
    register(context, FLUID_MAGIC, new DamageType("fluid.magic", 0.1f, DamageEffects.HURT));
    register(context, WATER, new DamageType("fluid.water", 0.1f, DamageEffects.DROWNING));
    register(context, FLUID_SPIKE, new DamageType("fluid.spike", 0.1f, DamageEffects.THORNS));
  }
