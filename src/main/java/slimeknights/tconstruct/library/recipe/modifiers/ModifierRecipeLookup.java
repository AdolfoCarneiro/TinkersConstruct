...
public class ModifierRecipeLookup {
  ...
  public static List<ModifierEntry> getRecipeModifierList() {
    ...
    if (RECIPE_MODIFIER_LIST == null) {
      RECIPE_MODIFIER_LIST = RECIPE_MODIFIERS.values().stream().distinct().sorted(Comparator.comparing(LazyModifier::getId)).map(mod -> new ModifierEntry(mod.getId(), 1)).toList();
    }
