...
public class ToolBuildingRecipeBuilder extends AbstractRecipeBuilder<ToolBuildingRecipeBuilder> {
  ...
  public void save(RecipeOutput consumerIn, ResourceLocation id) {
    ...
    if (tippedModifier != null) {
      ...
      consumerIn.accept(id, new TippedToolTransformRecipe(group, output, layoutSlot, extraRequirements.get(0), extraMaterials, tippedModifier), advancementId);
    } else {
      consumerIn.accept(id, new ToolBuildingRecipe(group, output, outputSize, layoutSlot, extraRequirements, partsOverride, extraMaterials), advancementId);
    }
  }
}
