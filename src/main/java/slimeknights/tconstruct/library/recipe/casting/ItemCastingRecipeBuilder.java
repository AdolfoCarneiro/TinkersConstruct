...
public class ItemCastingRecipeBuilder extends AbstractRecipeBuilder<ItemCastingRecipeBuilder> {
  ...
  public void save(RecipeOutput output) {
    this.save(output, BuiltInRegistries.ITEM.getKey(this.result.get().getItem()));
  }
  ...
  public void save(RecipeOutput output, ResourceLocation id) {
    ...
    if (result == ItemOutput.EMPTY) {
      ...
      output.accept(id, new CastDuplicationRecipe(recipeSerializer, group, cast, fluid, coolingTime), advancementHolder);
    } else {
      output.accept(id, new ItemCastingRecipe(recipeSerializer, group, cast, fluid, result, coolingTime, consumed && cast != Ingredient.EMPTY, switchSlots), advancementHolder);
    }
  }
}
