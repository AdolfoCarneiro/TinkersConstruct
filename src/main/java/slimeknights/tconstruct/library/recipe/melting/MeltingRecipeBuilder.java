...
public class MeltingRecipeBuilder extends AbstractRecipeBuilder<MeltingRecipeBuilder> {
  ...
  public void save(RecipeOutput consumer, ResourceLocation id) {
    ...
    AdvancementHolder advancementId = buildOptionalAdvancement(consumer, id, "melting");
    // based on properties, choose which recipe to build
    if (oreRate != null) {
      consumer.accept(id, new OreMeltingRecipe(group, input, output, temperature, time, byproducts, oreRate, byproductRates), advancementId);
    } else if (unitSizes != null) {
      consumer.accept(id, new DamageableMeltingRecipe(group, input, output, temperature, time, byproducts, unitSizes[0], List.of(Arrays.stream(unitSizes, 1, unitSizes.length).boxed().toArray(Integer[]::new))), advancementId);
    } else {
      consumer.accept(id, new MeltingRecipe(group, input, output, temperature, time, byproducts), advancementId);
    }
  }
}
