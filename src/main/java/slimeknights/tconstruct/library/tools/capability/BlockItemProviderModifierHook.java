...
public interface BlockItemProviderModifierHook {
    ...
    record CapabilityImpl(IToolStackView tool) implements BlockItemProviderCapability {
        ...
        public void consume(ItemStack capStack, ItemStack backingStack, @Nullable LivingEntity entity) {
            ...
            TConstruct.LOG.warn("Could not find a modifier to consume {} from after providing it from ToolBlockItemProviderHook. This is likely causing a duplication glitch! Stack nbt: {}", BuiltInRegistries.ITEM.getKey(backingStack.getItem()), backingStack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag());
        }
    }

    /** Capability provider, registered for all {@link slimeknights.tconstruct.library.tools.item.IModifiableDisplay} items by {@link ToolCapabilityProvider} */
    static BlockItemProviderCapability createIfPresent(ItemStack stack, @Nullable Void context) {
        return new CapabilityImpl(ToolStack.from(stack));
    }
}
