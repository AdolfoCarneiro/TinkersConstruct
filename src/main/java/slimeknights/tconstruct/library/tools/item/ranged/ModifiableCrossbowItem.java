...
public class ModifiableCrossbowItem extends ModifiableLauncherItem {
  ...
  protected void shootProjectile(LivingEntity shooter, Projectile projectile, int index, float velocity, float inaccuracy, float angle, @Nullable LivingEntity target) {
    // unused — crossbow fires via fireCrossbow, not the vanilla ProjectileWeaponItem mechanism
  }
  ...
  public void releaseUsing(ItemStack bow, Level level, LivingEntity living, int chargeRemaining) {
    ...
    if (!ammo.isEmpty()) {
      ...
      if (!level.isClientSide) {
        CompoundTag ammoNBT = (CompoundTag) ammo.save(level.registryAccess());
  ...
  public List<Component> getStatInformation(IToolStackView tool, @Nullable Player player, List<Component> tooltips, TooltipKey key, TooltipFlag tooltipFlag) {
    ...
    if (!heldAmmo.isEmpty() && player != null) {
      ItemStack heldStack = ItemStack.parseOptional(player.registryAccess(), heldAmmo);
