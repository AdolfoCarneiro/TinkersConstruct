...
public class TagIntersectionPresentCondition<T> implements ICondition {
  ...
  public static final MapCodec<TagIntersectionPresentCondition<?>> CODEC = RecordCodecBuilder.mapCodec(instance ->
    instance.group(
    ...
    ).apply(instance, (registry, tags) -> {
      ResourceKey<Registry<Object>> regKey = ResourceKey.createRegistryKey(registry);
      return new TagIntersectionPresentCondition<>(tags.stream().map(t -> TagKey.create(regKey, t)).toList());
    }));
