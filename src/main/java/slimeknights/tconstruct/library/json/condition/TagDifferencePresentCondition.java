...
public class TagDifferencePresentCondition<T> implements ICondition {
  ...
  public static final MapCodec<TagDifferencePresentCondition<?>> CODEC = RecordCodecBuilder.mapCodec(instance ->
    instance.group(
    ...
    ).apply(instance, (registry, base, subtracted) -> {
      ResourceKey<Registry<Object>> regKey = ResourceKey.createRegistryKey(registry);
      return new TagDifferencePresentCondition<>(
        TagKey.create(regKey, base),
        subtracted.stream().map(t -> TagKey.create(regKey, t)).toList());
    }));
